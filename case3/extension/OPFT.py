...
class VisionRotaryEmbeddingFast(nn.Module):
    ...
    def forward(self, x):
        ...
        if (
            ...
            max_len = max(seq_len, self.pt_seq_len)
            cos, sin = self._compute_cos_sin(max_len, x.device, x.dtype)
...
class ObservationEmbedder(nn.Module):
    def __init__(self, obs_dim=1050, hidden_size=1024):
        super().__init__()
...
class FinalLayer(nn.Module):
    def __init__(self, dim, patch_size=(32, 32), out_chans=5):
        super().__init__()
...
class ShiftedConvFusion(nn.Module):
    ...
    def __init__(
        ...
        if use_learnable_weights:
            ...
            self.register_buffer("weight_param", self._create_gaussian_weight(ph, pw))
    ...
    def _create_gaussian_weight(self, ph, pw, sigma=0.5):

        center_h, center_w = ph / 2 - 0.5, pw / 2 - 0.5

        h = torch.arange(ph).float()
        w = torch.arange(pw).float()
    ...
    def _reconstruct_with_weights(self, patches, weights):
        ...
        output = torch.zeros(B, C, H, W, device=patches.device, dtype=patches.dtype)
        weight_accum = torch.zeros(
            1, 1, H, W, device=patches.device, dtype=patches.dtype
        )

        weights_expanded = weights.expand(B, C, -1, -1)
        ...
        for i in range(self.grid_h):
            for j in range(self.grid_w):
                ...
                patch_data = patches[:, idx, :, :, :]

                output[:, :, h_start:h_end, w_start:w_end] += (
                    patch_data * weights_expanded
                )
                weight_accum[:, :, h_start:h_end, w_start:w_end] += weights[0:1, 0:1]
...
class JiTWithShiftedConv(nn.Module):
    def __init__(
        ...
        self.rope = VisionRotaryEmbeddingFast(head_dim, num_patches)
    ...
    def forward(self, x, t, obs):
        ...
        recon_img = self.fusion_layer(patches)
...
JiT_MODELS = {
    "jit_small": lambda **kwargs: JiTWithShiftedConv(
        depth=4, num_heads=4, embed_dim=1024, fusion_channels=64, **kwargs
    ),
