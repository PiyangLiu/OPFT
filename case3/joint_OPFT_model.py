...
from OPFT import OPFT_MODELS, ObservationEmbedder, TimestepEmbedder


class JointOPFT(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        if backbone not in OPFT_MODELS:
            raise KeyError(
                f"unknown OPFT backbone {backbone!r}; available: {tuple(OPFT_MODELS)}"
            )
        self.field_channels = field_channels
        self.observation_dim = observation_dim
        self.relperm_dim = relperm_dim
        self.spatial_model = OPFT_MODELS[backbone](
            img_size=image_size,
